import UNET3D_binary,UNET3D_Multi, loss_functions, writegraphs
import Managefiles3D, ManageFilesMulti
...
MODEL_TRAIN_SAVE='/Notebooks/Marianadissertation/Humans/2D-UNET_Brain_Seg/trainingVisualize/models'
MODEL_FINAL_SAVE='/Notebooks/Marianadissertation/Rats/pythonrats3D/final_'
MODEL_SAVE_BEST='/Notebooks/Marianadissertation/Rats/pythonrats3D/bestvalueslasttrain'
...
def definetrainfunctions():
	if OPTIMIZER==0:
		function_optmizer=optimizers.Adam(lr=learning_rate)
	elif OPTIMIZER==1:
...
def compiletrain(mris_training, masks_training, model,function_optmizer):
    print("\n ########### TRAINING ############\n")
    #Choose in optmizer sgd, 
    all_metrics=['accuracy',loss_functions.Tversky_loss,loss_functions.Precision,loss_functions.FPR,loss_functions.FNR,loss_functions.specificity,loss_functions.sensitivity]
    model.compile(loss=loss_functions.dice_coef_multilabel, optimizer=function_optmizer, metrics=all_metrics)
    #Save graphs
    ...
    #Early Stoppping
    e1=EarlyStopping(monitor='val_loss', min_delta=0, patience=50, verbose=0, mode='auto')
    e2=EarlyStopping(monitor='val_acc', min_delta=0, patience=50, verbose=0, mode='auto')
    #Fit model with images, begin training
    history = model.fit(mris_training, masks_training, batch_size=BATCH_SIZE, epochs=EPOCHS , verbose=1, validation_split=0.15,initial_epoch=0, callbacks=[tensorboard,m1,m2,m3,m4,e1,e2])
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    model.save("/Notebooks/Marianadissertation/Rats/pythonrats3D/logs/model/" + timestamp + ".h5")
...
def evaluate(mris_evaluate,masks_evaluate,model,history):
    ...
    with open("/Notebooks/Marianadissertation/Rats/pythonrats3D/logs/model/log.csv", "a") as myfile:
        myfile.write(",")
...
if __name__ == '__main__':
    ...
    layers=UNET3D_Multi.my3DUnet()
    model=layers.create_DL
    print(model.summary())
    ######  organize files
    #BRAIN CLASS
    files=ManageFilesMulti.myFiles3D(IMAGE_MRI_PATH,IMAGE_MASK_PATH,IMAGE_MASK_BIN_PATH)
