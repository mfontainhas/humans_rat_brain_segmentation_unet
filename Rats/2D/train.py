...
##################  BEGIN VARIABLES  #################################
#Prepare files info
FILES_PREP=False #True-Prepare files from nii    ;   False - Open already saved numpys
N_FILES=144
IMAGE_MRI_PATH='/Notebooks/Marianadissertation/Rats/images/MRI'
IMAGE_MASK_PATH='/Notebooks/Marianadissertation/Rats/images/MASK' #"MASK" for Brain Seg or "MASKC" for Brain Classification
MODEL_TRAIN_SAVE='/Notebooks/Marianadissertation/Rats/pythonrats2D/trainingVisualize/models'
...
def compiletrain(mris_training, masks_training, model,function_optmizer):
    ...
    all_metrics=['accuracy',loss_functions.dice_coef_loss,loss_functions.Precision,loss_functions.FPR,loss_functions.FNR,loss_functions.specificity,loss_functions.sensitivity] #loss_functions.dice_coef_loss
    model.compile(loss=loss_functions.dice_coef_loss, optimizer=function_optmizer, metrics=all_metrics)
    #Save graphs
    ...
    history = model.fit(mris_training, masks_training, batch_size=BATCH_SIZE, epochs=EPOCHS , verbose=1, validation_split=0.2,initial_epoch=0, callbacks=[tensorboard,m1,m2,m3,m4,e1,e2],shuffle=False)
...
if __name__ == '__main__':
    ...
    files=Managefiles2D.myFiles2D(IMAGE_MRI_PATH,IMAGE_MASK_PATH,N_FILES)
    mris=files.openlist("mri")
    masks=files.openlist("mask")
    ...
    model, history = compiletrain(mris, masks, model,function_optmizer)
